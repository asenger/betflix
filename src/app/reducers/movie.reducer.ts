import { ActionEx, MovieActionTypes } from '../actions/movie.action';
...
export function MovieReducer(state = initialState, action: ActionEx) {
    switch (action.type) {
        ...
        case MovieActionTypes.LOAD_SELECTED_MOVIE: {
            const id = action.payload;
            const selectedMovie = state.allMovies.find(movie => movie.id === id);
        ...
        case MovieActionTypes.FILTER_MOVIES: {
            const { comparator, target, forceLowerCase } = action.payload;
